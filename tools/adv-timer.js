...
const { sleep, log, printFormattedTitle } = require('./utils/utils');
...
(async () => {
    ...
    for (let x in arr) {
        let data = await fetchData(`https://api.adsgram.ai/adv?blockId=2994&tg_id=${arr[x]}&tg_platform=web&platform=Win32&language=en`, 'get', { headers });
        if (data) {
            printFormattedTitle(`tài khoản ${x}`)
            let tracking1 = await fetchData(data.banner.trackings[0].value, 'get', { headers });
            log(`xong nv 1 [${JSON.stringify(tracking1)}]`)
            let tracking2 = await fetchData(data.banner.trackings[1].value, 'get', { headers });
            ...
            await sleep(5000)
            await sleep(5000)
            let tracking3 = await fetchData(data.banner.trackings[3].value, 'get', { headers });
