...
class Plugin extends PuppeteerExtraPlugin {
  ...
  async beforeLaunch(options) {
    ...
    // code nay toi them vao
    // if (this.opts.address == null || this.opts.address == 'null') {
    //   options.args = options.args.filter(arg => !arg.includes('--proxy-server'));
    // }
    return options;
  }
  ...
  async onPageCreated(page) {
    if (this.opts.credentials === Object(this.opts.credentials)) {
      ...
      return page.authenticate({ username: 'vhpahdvn', password: 'naarrlkx5cx3' });
    } else {
